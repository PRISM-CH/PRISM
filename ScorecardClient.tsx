...
export default function ScorecardClient({ data }: Props) {
  ...
  const radarData = pillars.map(p => ({
    name: p.name.replace(' & ', ' &\n'),
    FEI: Math.round((p.objectives as any[])?.reduce((a: number, o: any) => a + o.score, 0) / ((p.objectives as any[])?.length || 1)),
    Benchmark: 70,
  }))
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1.25rem 4rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ fontFamily: 'sans-serif', fontSize: 11, fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: 8 }}>
          PRISM Assessment · {assessment.assessment_year} · {assessment.methodology_version}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: '1.5rem' }}>
        {pillars.map((p, i) => {
          const objs = (p.objectives as any[]) || []
          const avg = Math.round(objs.reduce((a: number, o: any) => a + o.score, 0) / (objs.length || 1))
          ...
          return (
            <button
              ...
              style={{
                ...
                border: isActive ? `1.5px solid ${p.color}` : '0.5px solid var(--border)',
      ...
      {/* Detail panel */}
      {activePillar && (
        <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.25rem', marginBottom: '1.5rem', boxShadow: 'var(--shadow)' }}>
          ...
          {((activePillar.objectives as any[]) || []).map((obj: any) => (
      ...
      {/* Radar chart */}
      <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.25rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ fontFamily: 'sans-serif', fontSize: 12, color: 'var(--text2)', marginBottom: '1rem' }}>FEI 2025 · ASOIF benchmark average</div>
        <div style={{ display: 'flex', gap: 20, marginBottom: '1rem' }}>
          {[
            { label: 'FEI 2025', color: '#378ADD' },
            { label: 'IF benchmark avg', color: '#aaa', dash: true },
          ].map(l => (
        ...
        <ResponsiveContainer width="100%" height={280}>
          <RadarChart data={radarData}>
            ...
            <Radar name="FEI 2025" dataKey="FEI" stroke="#378ADD" fill="#378ADD" fillOpacity={0.12} strokeWidth={2} dot={{ r: 4, fill: '#378ADD' }} />
      ...
      <div style={{ fontFamily: 'sans-serif', fontSize: 11, color: 'var(--text3)', lineHeight: 1.7, borderTop: '0.5px solid var(--border)', paddingTop: 16 }}>
        ...
        <br />
        Sources: FEI Strategy 2025–2029 (published); FEI Annual Report 2023; IOC Basic Universal Principles of Good Governance; ASOIF IF Governance Review 2023; FEI.org public data.
      </div>
